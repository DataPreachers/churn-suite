...
class CoxWorkingPipeline:
    ...
    def _populate_cox_tables(self, stage0_data: pd.DataFrame, survival_panel: pd.DataFrame, 
        ...
        try:
            ...
            prioritization_records = self._populate_cox_prioritization_table(db, cox_data, model_data, actual_experiment_id)

            # 2b. customer_cox_details - kundenscharfe Ergebnis-Tabelle (kanonisch)
            self._populate_customer_cox_details(db, prioritization_records, actual_experiment_id, results)
    ...
    def _populate_cox_prioritization_table(self, db: ChurnJSONDatabase, cox_data: pd.DataFrame, 
                                         model_data: pd.DataFrame, experiment_id: int) -> list:
        """Befüllt cox_prioritization_results Tabelle mit Risiko-Scores und liefert Records zurück"""
        ...
        return prioritization_records

    def _populate_customer_cox_details(self, db: ChurnJSONDatabase, prioritization_records: list,
                                       experiment_id: int, results: Dict[str, Any]) -> None:
        """Befüllt customer_cox_details Tabelle (idempotent je experiment_id)"""
        self.logger.info("📊 Befülle customer_cox_details Tabelle (kanonisches Cox-Ergebnis)")

        # Tabelle sicherstellen
        tables = db.data.setdefault("tables", {})
        if "customer_cox_details" not in tables:
            tables["customer_cox_details"] = {"records": [], "schema": {}}

        existing_records = tables["customer_cox_details"].get("records", [])
        # Idempotenz: entferne alte Records für dieses Experiment
        filtered_existing = [r for r in existing_records if int(r.get("experiment_id", -1)) != int(experiment_id)]

        # Hilfsfunktion für Risikokategorie
        ...
        # Feature-Anzahl aus Ergebnissen, falls verfügbar
        feature_count = 0
        try:
            feature_count = int(results.get("performance", {}).get("model_performance", {}).get("features_count", 0))
        except Exception:
            feature_count = 0

        # Records aufbauen
        customer_details = []
        analysis_ts = results.get("timestamp")
        cutoff_exclusive = results.get("cutoff_exclusive")
        for rec in prioritization_records:
            score = float(rec.get("PriorityScore", 0.0))
            p12 = float(rec.get("P_Event_12m", 0.0))
            months_cond = float(rec.get("MonthsToLive_Conditional", 0.0))
            customer_details.append({
                "Kunde": int(rec.get("Kunde")),
                "experiment_id": int(experiment_id),
                "cox_analysis_type": "working_features",
                "feature_count": feature_count,
                "priority_score": score,
                "risk_category": _risk_category(score),
                "survival_probability_12m": max(0.0, min(1.0, 1.0 - p12)),
                "expected_lifetime_months": months_cond,
                "analysis_date": analysis_ts,
                "cutoff_date": cutoff_exclusive
            })

        # Zusammenführen und schreiben
        tables["customer_cox_details"]["records"] = filtered_existing + customer_details
        self.logger.info(f"✅ customer_cox_details: {len(customer_details)} Records (Experiment {experiment_id})")
