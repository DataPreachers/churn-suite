...
async def get_file_timebase(file_id: int):
    ...
    try:
        ...
        for rec in raw_tbl:
            try:
                fid = _to_int_safe(rec.get("file_id"))
                if fid is None or fid != target_id:
                    continue
    ...
    try:
        ...
        for existing in existing_experiments:
            if (existing.get("model_type") == experiment.model_type and
                ...
                existing.get("file_id") == int(experiment.id_file)):
        ...
        experiment_data = {
            ...
            "backtest_from": experiment.backtest_from,
            "backtest_to": experiment.backtest_to,
            "file_id": int(experiment.id_file),
            "status": "created",
            "created_at": datetime.now().isoformat(),
            "updated_at": datetime.now().isoformat()
        }
        
        # Validierung: file_id muss existieren und stage0_cache sein
        file_rec = None
        try:
            file_rec = json_db.get_file_by_id(int(experiment.id_file))
        except Exception:
            file_rec = None
        if not file_rec:
            raise HTTPException(status_code=400, detail=f"file_id {experiment.id_file} nicht gefunden")
        if (file_rec.get("source_type") or "").lower() != "stage0_cache":
            raise HTTPException(status_code=400, detail=f"file_id {experiment.id_file} ist kein stage0_cache Eintrag")
        ...
        experiment_id = json_db.create_experiment(
            ...
            model_type=experiment.model_type,
            feature_set=experiment.feature_set,
            id_file=int(experiment.id_file)
        )
...
async def update_experiment(experiment_id: int, experiment: ExperimentUpdate):
    ...
    try:
        ...
        if experiment.id_file is not None:
            # Validierung file_id
            file_rec = None
            try:
                file_rec = json_db.get_file_by_id(int(experiment.id_file))
            ...
            if (file_rec.get("source_type") or "").lower() != "stage0_cache":
                raise HTTPException(status_code=400, detail=f"file_id {experiment.id_file} ist kein stage0_cache Eintrag")
            update_data["file_id"] = int(experiment.id_file)
        ...
        updated_experiment = json_db.get_experiment_by_id(experiment_id)
