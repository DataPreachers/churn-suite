...
def _inject_saved_views(sql: str, db: ChurnJSONDatabase) -> str:
    """Injiziert nur die gespeicherten Views, die in der Query referenziert sind (inkl. Dependencies)."""
    try:
        all_views = db.list_views() or []
    except Exception:
        all_views = []
    if not all_views:
        return sql
    ...
    for v in all_views:
        ...
        if name and query:
            name_to_query[name] = query
    if not name_to_query:
        return sql

    import re as _re
    sql_text = sql or ""
    needed: set[str] = set()
    # Direkt referenzierte Views
    for name in name_to_query.keys():
        if _re.search(r"\b" + _re.escape(name) + r"\b", sql_text, flags=_re.IGNORECASE):
            needed.add(name)
    if not needed:
        return sql

    # Dependencies sammeln
    dep_graph: Dict[str, set[str]] = {n: set() for n in name_to_query.keys()}
    for n, q in name_to_query.items():
        for m in name_to_query.keys():
            if n == m:
                continue
            if _re.search(r"\b" + _re.escape(m) + r"\b", q, flags=_re.IGNORECASE):
                dep_graph[n].add(m)
    ...
    def visit(node: str):
        ...
        if node in temp:
            return
        temp.add(node)
        for d in dep_graph.get(node, set()):
            if d in name_to_query:
                visit(d)
        perm.add(node)
    ...
    inject_order = [n for n in resolved if n in name_to_query]
    if not inject_order:
        return sql
    ctes: List[str] = [f"{n} AS ({name_to_query[n]})" for n in inject_order]
    if not ctes:
        return sql
...
def _attach_lineage(rows: List[Dict[str, Any]], db: ChurnJSONDatabase) -> List[Dict[str, Any]]:
    if not rows:
        return rows
    # Wenn file_id bereits vorhanden ist, nichts anreichern
    if any("file_id" in r for r in rows):
        return rows
    # Heuristik: Für backtest_results KEINE Lineage anreichern
    sample = rows[0] if rows else {}
    if isinstance(sample, dict) and ("churn_probability" in sample or "risk_level" in sample):
        return rows
    # Enrich mit experiments.file_id (FK)
    exp_table = db.data.get("tables", {}).get("experiments", {})
    exp_records: List[Dict[str, Any]] = exp_table.get("records", []) or []
    if not exp_records:
        return rows
    # Lookup experiment_id -> file_id
    ...
    for exp in exp_records:
        ...
        fid = exp.get("file_id")
        try:
            id_map[key] = int(fid) if fid is not None else None
        except Exception:
            id_map[key] = None
    # Welcher Key im Resultat trägt die Experiment-ID?
    key_in_row = None
    ...
    for r in rows:
        ...
        try:
            ...
            exp_id = None
        if exp_id is not None and "file_id" not in new_r:
            fid = id_map.get(exp_id)
            if fid is not None:
                new_r["file_id"] = fid
...
def create_experiment():
    if not _check_password():
        return jsonify({"error": "Unauthorized"}), 403
    payload = request.get_json(silent=True) or {}
    required = ["experiment_name", "training_from", "training_to", "backtest_from", "backtest_to", "id_file"]
    ...
    try:
        db = _open_db()
        # Duplikat-Prüfung: gleiche Kombination aus Zeiträumen, Feature-Set, Modelltyp und file_id
        try:
            ...
            try:
                criteria_file_id = int(payload.get("id_file")) if payload.get("id_file") is not None else None
            except Exception:
            ...
            for ex in existing_records:
                try:
                    if (
                        ...
                        (ex.get("file_id") if ex.get("file_id") is not None else None) == criteria_file_id
                    ):
        ...
        exp_id = db.create_experiment(
            ...
            id_file=int(payload.get("id_file"))
        )
        db.save()
